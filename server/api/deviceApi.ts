...
import { ResultCommon, DeviceResult } from 'achieve-it-contract'
...
// getDevice
router.get("/:device_id", (req, res: Response<DeviceResult | ResultCommon>) => {
        ...
        if (err) {
            ...
            conn.query($sql.device.updateDeviceById,
                [device_details.device_name || old_device.device_name,
                    device_details.device_status || old_device.device_status, device_id], (err2, result2) => {
                    if (err2) {
                        res.json({
                            status: "error",
