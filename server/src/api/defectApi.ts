...
// get /defect/getAllOfMember/:member_id
router.get('/getAllOfMember/:member_id', (req, res) => {
  const member_id = req.params.member_id;
  conn.query('select * from defect a inner join member_project b on a.project_id = b.project_id where b.member_id = ?', [member_id], (err, result) => {
    if (err) {
      mysqlErrorHandler(res, err);
    } else {
