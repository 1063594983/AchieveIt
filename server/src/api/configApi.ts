...
router.put("/:project_id", (req, res: Response<ResultCommon>) => {
    ...
    conn.query($sql.config.getConfigByProjectId, [project_id], (err, result) => {
        if (err) {
            ...
            const old = result[0];
            conn.query($sql.config.updateConfigByProjectId, [details.git_address || old.git_address, 
                details.server_menu || old.server_menu, details.vm_space || old.vm_space, project_id], (err2) => {
